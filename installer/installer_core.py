"""FlowHub â€” Installer Core (B4 Foundation)
...
- No A2 Platform Core imports â€” one-way dependency rule
...
class InstallerConfig:
    ...
    postgres_password: str = ""   # empty â†’ auto-generate
    jwt_secret: str = ""           # empty â†’ auto-generate
    rest_api_secret: str = ""      # empty â†’ auto-generate
    timezone: str = "UTC"
    currency: str = "USD"
...
@dataclass
class DryRunResult:
    """What the installer would do â€” produced without writing any files."""
...
class InstallerRollback:
    ...
    On rollback(), removes only tracked paths â€” never touches pre-existing
...
def generate_secrets() -> InstallerSecrets:
    ...
    Returns an InstallerSecrets â€” call .masked_summary() for display.
    """
...
def check_prerequisites(install_dir: Path | None = None) -> list[PrerequisiteResult]:
    ...
    Checks command *availability* only â€” does NOT execute Docker, does NOT
...
def generate_env_content(config: InstallerConfig) -> str:
    ...
    Does NOT write any file â€” returns the content as a string.
...
def generate_toml_content(config: InstallerConfig) -> str:
    ...
    Secrets are NOT included â€” they live only in the .env file.
    ...
    created_at = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    lines = [
        "# FlowHub â€” managed configuration file",
        f"# Generated by installer on {created_at}",
        "# DO NOT EDIT MANUALLY. Use: flowhub configure (B5)",
        "# Secrets are NOT stored here â€” see .env (mode 600)",
        "",
        "[meta]",
        'version = "beta-1.0.0"',
...
def write_toml_config(
    ...
    """Write managed TOML config to config_dir/flowhub.toml.

    config_dir must exist (created by setup_storage). Tracks new file in rollback.
    """
    config_path = config_dir / "flowhub.toml"
...
def validate_generated_config(
    ...
    check_paths=False by default â€” storage dirs may not exist yet at generation time.
    Never calls sys.exit â€” returns ValidationResult; caller decides what to do.
    """
    if env_dict is None:
...
def dry_run_install(config: InstallerConfig, install_dir: Path) -> DryRunResult:
    ...
    return DryRunResult(
        ...
        files_would_be_written=[
            install_dir / ".env",
            Path(config.storage_path) / "config" / "flowhub.toml",
        ],
    )
...
def build_confirmation_summary(config: InstallerConfig) -> str:
    ...
    Secrets are masked â€” never shown in plain text.
    """
    ...
    lines = [
        "â”پ" * 56,
        "  FlowHub â€” Installation Summary",
