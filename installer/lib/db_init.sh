#!/usr/bin/env bash
# FlowHub - Database initialization and migration runner
#
# Source from install.sh. Requires:
#   - PostgreSQL container healthy (call wait_for_postgres_ready first)
#   - FLOWHUB_POSTGRES_USER, FLOWHUB_POSTGRES_DB exported in environment
#
...
#   - No rollback logic
#   - Targets FLOWHUB environment ONLY (never production)
...
run_alembic_migrations() {
    ...
    echo "  Command: alembic -c alembic_flowhub.ini upgrade head"

    if ! ${dc_cmd} --project-directory "$install_dir" -f "$compose_file" exec -T app \
        alembic -c alembic_flowhub.ini upgrade head; then
        echo "" >&2
        ...
        echo "    docker compose -f ${compose_file} exec app alembic -c alembic_flowhub.ini current" >&2
        return 1
    fi
