...
function makeUser(overrides: Partial<AuthUser> = {}): AuthUser {
  return {
    ...
    permissions: {
      ...
      [WORKSPACE_PERMISSION.read]: true,
      [WORKSPACE_PERMISSION.create]: true,
      [WORKSPACE_PERMISSION.edit]: true,
    },
    ...overrides,
  }
}
...
describe('RequirePermission - direct render', () => {
  ...
  it('evaluates canonical Workspace permissions independently of legacy site aliases', () => {
    const user = makeUser({ permissions: { can_access_site: false, [WORKSPACE_PERMISSION.read]: true } })
    const c = renderAuth(
      <MemoryRouter>
        <RequirePermission permission={WORKSPACE_PERMISSION.read}>
          <span>workspace-read</span>
        </RequirePermission>
      </MemoryRouter>,
      makeAuth(user),
    )
    expect(c.textContent).toContain('workspace-read')
  })
...
describe('Sidebar - admin user (is_admin=true)', () => {
  ...
  it('renders centralized icons for active navigation links', () => {
    ...
    expect(dashboardIcon?.className).toContain('fh-icon-lg')
    expect(dashboardIcon?.getAttribute('style')).toBeNull()
    expect(dashboardIcon?.querySelector('svg[stroke="currentColor"]')).not.toBeNull()
