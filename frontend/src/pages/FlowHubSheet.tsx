...
export default function FlowHubSheet() {
  ...
  async function save() {
    ...
    try {
      ...
      notify.success({ title: translate('flowhubSheet:flowHubSheet.sheetRevisionSaved'), description: translate('flowhubSheet:flowHubSheet.revisionIsImmutableAndReadyForSnapshot', { value1: updated.version }) })
    } catch (error) { notify.error({ title: translate('flowhubSheet:flowHubSheet.sheetWasNotSaved'), description: localizedApiError(error, 'flowhubSheet:flowHubSheet.tryAgain') }) }
  }
  ...
  return <PageShell>
    ...
    <section className="fh-card" aria-label={translate('flowhubSheet:flowHubSheet.flowhubSheetEditor')}>
      ...
      <div className="flex items-center justify-between border-t border-border p-4"><span className="fh-text-caption">{Object.keys(changes).length} {translate('flowhubSheet:flowHubSheet.unsavedCells')} {Object.keys(columnNames).length} {translate('flowhubSheet:flowHubSheet.renamedColumnsPage')} {page} {translate('flowhubSheet:flowHubSheet.of')} {pageCount}</span><div className="flex gap-2"><button className="fh-button-secondary fh-button-sm" type="button" disabled={page <= 1 || Object.keys(changes).length > 0 || Object.keys(columnNames).length > 0} onClick={() => setPage(value => value - 1)}>{translate('flowhubSheet:flowHubSheet.previous')}</button><button className="fh-button-secondary fh-button-sm" type="button" disabled={page >= pageCount || Object.keys(changes).length > 0 || Object.keys(columnNames).length > 0} onClick={() => setPage(value => value + 1)}>{translate('flowhubSheet:flowHubSheet.next')}</button></div></div>
    </section>
  </PageShell>
}
