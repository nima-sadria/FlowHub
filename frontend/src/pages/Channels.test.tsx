...
describe('Channels page', () => {
  ...
  it('shows configured channel secrets as write-only and preserves them when Save submits blanks', async () => {
    ...
    expect(secretInputs.every(input => input.placeholder === '')).toBe(true)
    ...
    const revealButtons = Array.from(dialog.querySelectorAll<HTMLButtonElement>('[aria-label="Show entered secret"]'))
    const copyButtons = Array.from(dialog.querySelectorAll<HTMLButtonElement>('[aria-label="Copy entered secret"]'))
    expect(revealButtons.every(button => button.disabled)).toBe(true)
    expect(copyButtons.every(button => button.disabled)).toBe(true)
    expect([...revealButtons, ...copyButtons].every(button => button.title === 'Saved secret is hidden for security — type a new one to reveal or copy it.')).toBe(true)

    act(() => secretInputs[0].focus())
    expect(secretInputs[0].placeholder).toBe('Type your password')
    expect(dialog.querySelectorAll('[data-testid="configured-secret-mask"]')).toHaveLength(1)
    act(() => secretInputs[0].blur())
    expect(dialog.querySelectorAll('[data-testid="configured-secret-mask"]')).toHaveLength(2)
