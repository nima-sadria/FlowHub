    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          setError(data.detail ?? 'Invalid credentials. Please check your email and password.')
    ...
    <div className="flex min-h-screen flex-col items-center justify-center gap-8 bg-bg-base px-4 py-6 sm:px-8">
      ...
      <main className="w-full max-w-[460px] rounded-lg border border-border bg-bg-card p-6 shadow-[0_8px_24px_-4px_rgba(15,23,41,0.1)] sm:p-10">
        <div className="text-center">
          <h1 className="text-2xl font-semibold leading-[30px] text-text-base">Sign in to FlowHub</h1>
          <p className="mt-4 text-[13px] leading-[22px] text-[color:var(--fh-text-secondary)]">Use your workspace account.</p>
        </div>
        ...
        <form onSubmit={event => { void handleSubmit(event) }} className="mt-6 flex flex-col gap-4">
          <div className="fh-field">
            <label htmlFor="login-identifier" className="fh-label">Email</label>
            <input
              id="login-identifier"
              type="text"
              value={identifier}
              ...
              disabled={loading}
              className="fh-input"
              {...inputHint('owner@flowhub.app')}
            />
          </div>

          <div className="fh-field">
            <label htmlFor="login-password" className="fh-label">Password</label>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="text-xs font-medium leading-4 text-accent">Forgot password?</span>
          </div>
      ...
      <footer className="text-center text-[11px] leading-4 text-wp-muted">Privacy · Security · Support</footer>
    </div>
  )
}
