...
export default function DataQuality() {
  ...
  return <PageShell>
    ...
    <div className="mt-5 grid gap-3">{grouped.length === 0 ? <div className="fh-card fh-card-pad"><p className="font-medium text-text-base">{translate('dataQuality:dataQuality.noDataQualityIssues')}</p><p className="fh-text-caption">{translate('dataQuality:dataQuality.sourceAnalysisHasNotRecordedAnyMatching')}</p></div> : grouped.map(([key, items]) => { const issue = items[0]; return <details className="fh-card fh-card-pad" key={key}><summary className="flex cursor-pointer items-center gap-3"><Icon name={issue.severity === "warning" ? "warning" : "alert"} /><span className="font-medium text-text-base">{issue.category.replace(/_/g, ' ')}</span><span className="fh-badge fh-badge-neutral">{items.length}</span><span className="ms-auto fh-text-caption">{issue.channelId ? formatChannelDisplayName(issue.channelId) : translate('dataQuality:dataQuality.allChannels')}</span></summary><div className="mt-4 grid gap-3">{items.map(item => <article className="rounded border border-border p-3" key={item.id}><div className="flex flex-wrap gap-2"><p className="font-medium text-text-base">{item.summary}</p>{item.sourceProductName && <span className="fh-badge fh-badge-neutral">{item.sourceProductName}</span>}{item.mappingState && <span className="fh-badge fh-badge-neutral">{translate('dataQuality:dataQuality.mapping')} {item.mappingState}</span>}</div><p className="fh-text-caption mt-1">{translate('dataQuality:dataQuality.recommendedAction')} {item.recommendedAction}</p><details className="mt-2"><summary className="fh-text-caption cursor-pointer">{translate('dataQuality:dataQuality.technicalDetails')}</summary><pre className="mt-2 overflow-auto rounded bg-bg-base p-2 text-xs">{JSON.stringify(item.technicalDetails, null, 2)}</pre></details></article>)}</div></details> })}</div>
  </PageShell>
}
