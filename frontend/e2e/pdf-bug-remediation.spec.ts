...
test.describe.serial('PDF usability remediation with a fully isolated synthetic backend', () => {
  ...
  test('Source configuration presents shared and independent worksheet rules with per-Channel columns', async ({ page }) => {
    for (const viewport of viewports) {
      ...
      await expect(wooColumns).toContainText('WooCommerce Primary')
      await expect(snapColumns).toContainText('SnappShop Main')
      await expect(tapsiColumns).toContainText('TapsiShop Main')
      await expect(page.locator('[data-channel-id="digikala:future"]')).toContainText('Digikala')
