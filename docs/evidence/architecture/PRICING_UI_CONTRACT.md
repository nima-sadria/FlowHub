...
- **`docs/development/contracts/FRONTEND_CONTRACT.md` (development contract) is the authoritative, callable
...
Any frontend that must call the backend uses `docs/development/contracts/FRONTEND_CONTRACT.md`. Where this
document and `docs/development/contracts/FRONTEND_CONTRACT.md` diverge for a surface that is callable now,
...
Raised by Claude UI Phase 1 while cross-checking `docs/development/contracts/FRONTEND_CONTRACT.md` (the
...
- **PM-1 — Response envelope and `contract_version`.** This document mandates a
  ...
  `docs/development/contracts/FRONTEND_CONTRACT.md` responses are bare JSON objects (for example
  ...
  the bare shapes from `docs/development/contracts/FRONTEND_CONTRACT.md` and does not assume an envelope.

- **PM-2 — Field-naming convention.** `docs/development/contracts/FRONTEND_CONTRACT.md` uses camelCase in
...
- **PM-3 — List pagination.** `docs/development/contracts/FRONTEND_CONTRACT.md` list endpoints
...
- **PM-4 — Monetary integers: number vs string.** `docs/development/contracts/FRONTEND_CONTRACT.md` shows
...
- **PM-5 — Nullability of lifecycle/head fields.** `docs/development/contracts/FRONTEND_CONTRACT.md` lists
...
- **PM-6 — Per-rule casing in the `PolicyRevision` response.**
  `docs/development/contracts/FRONTEND_CONTRACT.md` documents `rules[]` on the `PolicyRevision` response but
...
- **PM-7 — Where workspace preconditions are projected.** This document defines
  ...
  and `docs/development/contracts/FRONTEND_CONTRACT.md` exposes the primitives (`GET /units/...` returns
...
- **PM-8 — Authoritative scope reference for `global` unit declarations.**
  `docs/development/contracts/FRONTEND_CONTRACT.md`'s `GET/PUT /units/{scope}/{scopeReference}` documents
...
- **PM-9 — No authoritative product-identifier lookup for Pricing Matrix.**
  ...
  `POST /product-groups` takes `canonical_product_ids`, but `docs/development/contracts/FRONTEND_CONTRACT.md`
  ...
  `docs/archive/handoffs/UI_RESUME.md` for the UI-side note. If a canonical product lookup exists or
