...
| Current tests | `frontend/src/pages/layoutRules.test.ts` no longer lists `Workspace.tsx` as a primary page; 14 frontend test files had `workspace: {}` service stubs removed; no test currently asserts a first-class `/workspace` UI exists. |
