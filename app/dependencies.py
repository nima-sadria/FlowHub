"""FlowHub - FastAPI dependency injectors.

Provides injectable dependencies for all API route handlers.

Planned dependency functions:
    get_config()         - Yields the typed FlowHubConfig object from ConfigurationManager.
    ...
    get_current_user()   - Validates the JWT from Authorization header and returns
                           the authenticated FlowHubUser. Raises HTTP 401 if missing/invalid.
...
Usage (template_variable shape):
    @router.get("/example")
    async def example(
        config: FlowHubConfig = Depends(get_config),
