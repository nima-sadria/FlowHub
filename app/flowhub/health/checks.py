...
Tests inject TestDoubleNetworkAdapter; production injects a real adapter.
        ...
        try:
            ...
            if days < _TLS_WARN_DAYS:
                return HealthCheckResult.warn(
                    ...
                    message=f"TLS ok آ· cert expires in {days} days ({expiry})",
            ...
            return HealthCheckResult.ok(
                self.check_name, CheckCategory.TLS, target,
                message=f"TLS ok آ· cert expires {expiry} ({days} days)",
                duration_ms=ms,
                details=details,
            )
...
# ---------------------------------------------------------------------------
# 9. Docker Check (unavailable in CP1 - implemented in B6)
# ---------------------------------------------------------------------------
...
class DockerCheck:
    ...
    def run(self) -> HealthCheckResult:
        return HealthCheckResult.unavailable_skip(
            self.check_name,
