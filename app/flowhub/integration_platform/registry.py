...
_DEFINITIONS: dict[str, ConnectorDefinition] = {
    ...
    "digikala": ConnectorDefinition(
        connector=ConnectorDescriptor(
            ...
            capabilities=ConnectorCapabilities(
                # Exact read routes are documented, but their response and
                # query schemas are not. Do not claim normalized FlowHub
                # reads/cache/order sync until that contract is supplied.
                read_products=False,
                read_categories=False,
                read_inventory=False,
                read_orders=False,
        ...
        settings_schema=[
            ConnectorSettingDefinition(
                ...
                label="Base URL",
                required=False,
                default="https://seller.digikala.com/open-api/v1",
            ),
            ConnectorSettingDefinition(
                key="request_timeout",
    ...
    "technolife": ConnectorDefinition(
        ...
        settings_schema=[
            ConnectorSettingDefinition(key="base_url", label="Base URL", required=False, default="https://seller-api.technolife.com"),
            ConnectorSettingDefinition(key="request_timeout", label="Request timeout seconds", required=False, default=30),
            ConnectorSettingDefinition(key="api_key", label="API key", required=True, secret=True),
            ConnectorSettingDefinition(key="encryption_secret", label="Encryption secret", required=True, secret=True),
        ],
        diagnostics_contract=_diagnostics(
            ("settings", "configuration"),
            ("product_probe", "health"),
            ("health_record", "health"),
        ),
    ),
