...
class EnvironmentLoader:
    """Loads FLOWHUB_* environment variables from .env files and os.environ."""
    ...
    def load_FLOWHUB_only(self, env_file: Path | None = None) -> dict[str, str]:
        """Like load() but returns only FLOWHUB_* variables."""
        return {k: v for k, v in self.load(env_file).items() if k.startswith("FLOWHUB_")}
