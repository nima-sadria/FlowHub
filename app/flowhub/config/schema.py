...
FlowHubConfig is the typed, immutable configuration object returned by
...
class FlowHubConfig(BaseModel):
    ...
    # Network
    domain: str = Field(min_length=1, description="Hostname where FLOWHUB is served")
    ...
    log_level: str = Field(default=str(DEFAULTS["FLOWHUB_LOG_LEVEL"]))
    jwt_access_ttl_minutes: int = Field(
        default=int(DEFAULTS["FLOWHUB_JWT_ACCESS_TTL_MINUTES"]), ge=1
    )
    jwt_refresh_ttl_days: int = Field(
        default=int(DEFAULTS["FLOWHUB_JWT_REFRESH_TTL_DAYS"]), ge=1
    )
    max_upload_mb: int = Field(default=int(DEFAULTS["FLOWHUB_MAX_UPLOAD_MB"]), ge=1)
    worker_concurrency: int = Field(
        default=int(DEFAULTS["FLOWHUB_WORKER_CONCURRENCY"]), ge=1
    )
    ...
    @classmethod
    def from_env(cls, env: dict[str, str]) -> "FlowHubConfig":
        """Build FlowHubConfig from a flat env var dict (FLOWHUB_* keys).
        ...
        return cls(
            env=ConfigProfile.from_string(_get("FLOWHUB_ENV", "production")),
            domain=_get("FLOWHUB_DOMAIN"),
            port=_get_int("FLOWHUB_PORT", 0),
            ssl_mode=_get("FLOWHUB_SSL_MODE"),
            database_url=_get("FLOWHUB_DATABASE_URL"),
            postgres_db=_get("FLOWHUB_POSTGRES_DB"),
            postgres_user=_get("FLOWHUB_POSTGRES_USER"),
            postgres_password=_get("FLOWHUB_POSTGRES_PASSWORD"),
            jwt_secret=_get("FLOWHUB_JWT_SECRET"),
            rest_api_secret=_get("FLOWHUB_REST_API_SECRET"),
            nextcloud_url=_get("FLOWHUB_NEXTCLOUD_URL"),
            nextcloud_file_path=_get("FLOWHUB_NEXTCLOUD_FILE_PATH"),
            nextcloud_username=_get("FLOWHUB_NEXTCLOUD_USERNAME"),
            nextcloud_password=_get("FLOWHUB_NEXTCLOUD_PASSWORD"),
            woocommerce_url=_get("FLOWHUB_WOOCOMMERCE_URL"),
            woocommerce_key=_get("FLOWHUB_WOOCOMMERCE_KEY"),
            woocommerce_secret=_get("FLOWHUB_WOOCOMMERCE_SECRET"),
            timezone=_get("FLOWHUB_TIMEZONE"),
            currency=_get("FLOWHUB_CURRENCY"),
            admin_email=_get("FLOWHUB_ADMIN_EMAIL"),
            storage_path=storage_path,
            backup_path=_get("FLOWHUB_BACKUP_PATH"),
            plugin_dir=plugin_dir,
            log_level=_get("FLOWHUB_LOG_LEVEL", str(DEFAULTS["FLOWHUB_LOG_LEVEL"])),
            jwt_access_ttl_minutes=_get_int(
                "FLOWHUB_JWT_ACCESS_TTL_MINUTES", int(DEFAULTS["FLOWHUB_JWT_ACCESS_TTL_MINUTES"])
            ),
            jwt_refresh_ttl_days=_get_int(
                "FLOWHUB_JWT_REFRESH_TTL_DAYS", int(DEFAULTS["FLOWHUB_JWT_REFRESH_TTL_DAYS"])
            ),
            max_upload_mb=_get_int(
                "FLOWHUB_MAX_UPLOAD_MB", int(DEFAULTS["FLOWHUB_MAX_UPLOAD_MB"])
            ),
            worker_concurrency=_get_int(
                "FLOWHUB_WORKER_CONCURRENCY", int(DEFAULTS["FLOWHUB_WORKER_CONCURRENCY"])
            ),
