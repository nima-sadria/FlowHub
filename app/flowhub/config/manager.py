...
Public API:
    ...
    get()       - return typed FlowHubConfig; raise if not loaded or invalid
...
from .expander import expand_template_variables
...
_TOML_PATH_TO_ENV: dict[str, str] = {
    "app.env": "FLOWHUB_ENV",
    "app.domain": "FLOWHUB_DOMAIN",
    "app.port": "FLOWHUB_PORT",
    "app.timezone": "FLOWHUB_TIMEZONE",
    "app.currency": "FLOWHUB_CURRENCY",
    "app.storage_path": "FLOWHUB_STORAGE_PATH",
    "app.backup_path": "FLOWHUB_BACKUP_PATH",
    "app.ssl_mode": "FLOWHUB_SSL_MODE",
    "app.log_level": "FLOWHUB_LOG_LEVEL",
    "database.postgres_db": "FLOWHUB_POSTGRES_DB",
    "database.postgres_user": "FLOWHUB_POSTGRES_USER",
    "source.nextcloud_url": "FLOWHUB_NEXTCLOUD_URL",
    "source.nextcloud_file_path": "FLOWHUB_NEXTCLOUD_FILE_PATH",
    "source.nextcloud_username": "FLOWHUB_NEXTCLOUD_USERNAME",
    "channel.woocommerce_url": "FLOWHUB_WOOCOMMERCE_URL",
}
...
class ConfigurationManager:
    """Framework-independent configuration manager for FlowHub."""
    ...
    def __init__(
        ...
        self._config: FlowHubConfig | None = None
        self._loaded = False
    ...
    def load(self, env_file: Path | None = None) -> None:
        ...
        if self._config_file and self._config_file.exists():
            ...
            expanded = expand_template_variables(raw_toml, self._env)
    ...
    def get(self) -> FlowHubConfig:
        """Return the typed FlowHubConfig object.
        ...
        if self._config is None:
            self._config = FlowHubConfig.from_env(self._env)
        return self._config
    ...
    def set(self, key: str, value: str) -> None:
        ...
        Invalidates the cached FlowHubConfig and ValidationResult.
        """
        self._env[key] = value
        self._config = None
        self._validation_result = None
    ...
    def profile(self) -> ConfigProfile:
        """Return the current ConfigProfile (DEV, FLOWHUB, or PRODUCTION)."""
        if not self._loaded:
            self.load()
        env_value = self._env.get("FLOWHUB_ENV", "production")
    ...
    def _apply_defaults(self) -> None:
        ...
        if not self._env.get("FLOWHUB_PLUGIN_DIR") and self._env.get("FLOWHUB_STORAGE_PATH"):
            self._env["FLOWHUB_PLUGIN_DIR"] = f"{self._env['FLOWHUB_STORAGE_PATH']}/plugins"
