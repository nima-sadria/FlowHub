...
class LoggingCorrelation(BetaBase):
...
class LoggingRequestTrace(BetaBase):
...
class LoggingRetentionPolicy(BetaBase):
...
class LoggingExportEvent(BetaBase):
...
class LoggingRedactionPolicyVersion(BetaBase):
