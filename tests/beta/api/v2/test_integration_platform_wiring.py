...
os.environ.setdefault("BETA_JWT_SECRET", "test-ip-wiring-jwt-secret-32bytes!")
...
def auth_headers(client, db):
    ...
    user = BetaUser(username="ipadmin", hashed_password=hash_password("password123"), role="admin")
    db.add(user)
    db.commit()
...
def test_canonical_capability_schema_baseline():
    from app.beta.integration_platform.contracts import ConnectorCapabilities, ConnectorHealthStatus
...
def test_direct_call_audit_for_active_beta_v2_routes():
