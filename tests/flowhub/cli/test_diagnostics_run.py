...
def _make_test_double_runner(all_pass: bool = True):
...
class TestDiagnosticsRunCommand:
    ...
    def test_run_all_pass_exits_zero(self):
        _runner = _make_test_double_runner(all_pass=True)
        with mock.patch("cli.diagnostics.RealNetworkAdapter"), \
    ...
    def test_run_with_fail_exits_nonzero(self):
        _runner = _make_test_double_runner(all_pass=False)
        with mock.patch("cli.diagnostics.RealNetworkAdapter"), \
             mock.patch("cli.diagnostics.DiagnosticRunner", return_value=_runner):
            result = runner.invoke(app, ["diagnostics", "run"])
        assert result.exit_code != 0
    ...
    def test_run_no_secrets_in_output(self):
        ...
        from app.flowhub.diagnostics.runner import DiagnosticRunner
        test_double = TestDoubleNetworkAdapter()
        _runner = DiagnosticRunner(adapter=test_double, config={"FLOWHUB_NEXTCLOUD_PASSWORD": "secretpassword"})
