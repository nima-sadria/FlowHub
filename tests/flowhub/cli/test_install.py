...
class TestInstallDryRun:
    ...
    def test_production_profile_allows_read_only_dry_run(self, production_env_file: Path, tmp_path: Path):
        result = runner.invoke(app, [
            "install", "dry-run",
    ...
    def test_production_dry_run_shows_plan(self, production_env_file: Path, tmp_path: Path):
        result = runner.invoke(app, [
            "install", "dry-run",
            "--env-file", str(production_env_file),
            "--install-dir", str(tmp_path / "install"),
        ])
        assert "Files That Would Be Written" in result.output
