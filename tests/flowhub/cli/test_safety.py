...
class TestProductionProfileBlocks:
    def test_production_allows_read_only_install_dry_run(self, production_env_file: Path, tmp_path: Path):
        result = runner.invoke(app, [
            "install", "dry-run",
...
class TestEnvGuard:
    def test_require_FLOWHUB_env_raises_for_production(self):
        from cli.shared.env_guard import require_FLOWHUB_env, ProductionResourceError
        from app.flowhub.config import ConfigProfile
        try:
            require_FLOWHUB_env(ConfigProfile.PRODUCTION)
            assert False, "Should have raised ProductionResourceError"
        except ProductionResourceError:
            pass

    def test_require_FLOWHUB_env_passes_for_dev(self):
        from cli.shared.env_guard import require_FLOWHUB_env
        from app.flowhub.config import ConfigProfile
        require_FLOWHUB_env(ConfigProfile.DEV)  # Must not raise
