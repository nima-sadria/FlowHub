...
class TestGenerateTomlContent:
    ...
    def test_uses_template_variable_syntax_for_env_vars(self, valid_config_with_paths: InstallerConfig):
        content = generate_toml_content(valid_config_with_paths)
        assert "${FLOWHUB_ENV}" in content
        assert "${FLOWHUB_DOMAIN}" in content
        assert "${FLOWHUB_TIMEZONE}" in content

    def test_port_is_integer_not_template_variable(self, valid_config_with_paths: InstallerConfig):
        content = generate_toml_content(valid_config_with_paths)
    ...
    def test_has_version_marker(self, valid_config_with_paths: InstallerConfig):
        content = generate_toml_content(valid_config_with_paths)
        assert "FLOWHUB-1.0.0" in content
...
class TestNoA2Mutation:
    ...
    def test_installer_core_imports_only_from_app_FLOWHUB_config(self):
        import installer.installer_core as core
        source_file = Path(core.__file__)
        source = source_file.read_text(encoding="utf-8")
        # Only allowed app.flowhub import is app.flowhub.config
        lines_with_app = [l for l in source.splitlines() if "from app." in l or "import app." in l]
        for line in lines_with_app:
            assert "app.flowhub.config" in line, f"Unexpected app import: {line}"
