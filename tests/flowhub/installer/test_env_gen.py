...
class TestGenerateEnvContent:
    ...
    def test_contains_FLOWHUB_env(self, valid_config_with_paths: InstallerConfig):
        content = generate_env_content(valid_config_with_paths)
        assert "FLOWHUB_ENV=production" in content
    ...
    def test_contains_all_required_field_names(self, valid_config_with_paths: InstallerConfig):
        ...
        required = [
            ...
            "FLOWHUB_NEXTCLOUD_USERNAME", "FLOWHUB_NEXTCLOUD_PASSWORD",
    ...
    def test_constructs_database_url_from_parts(self, valid_config_with_paths: InstallerConfig):
        content = generate_env_content(valid_config_with_paths)
        assert "FLOWHUB_DATABASE_URL=postgresql://" in content
    ...
    def test_domain_value_in_content(self, valid_config_with_paths: InstallerConfig):
        content = generate_env_content(valid_config_with_paths)
        assert f"FLOWHUB_DOMAIN={valid_config_with_paths.domain}" in content

    def test_port_value_in_content(self, valid_config_with_paths: InstallerConfig):
        content = generate_env_content(valid_config_with_paths)
        assert f"FLOWHUB_PORT={valid_config_with_paths.port}" in content
...
class TestParseEnvContent:
    def test_round_trip(self, valid_config_with_paths: InstallerConfig):
        ...
        assert parsed["FLOWHUB_ENV"] == "production"
        assert parsed["FLOWHUB_DOMAIN"] == valid_config_with_paths.domain
        assert parsed["FLOWHUB_PORT"] == str(valid_config_with_paths.port)
