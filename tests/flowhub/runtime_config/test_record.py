...
from app.flowhub.runtime_config.record import (
    EDITABLE_FIELDS,
...
class TestFieldSets:
    ...
    def test_known_editable_fields(self):
        assert "FLOWHUB_LOG_LEVEL" in EDITABLE_FIELDS
        assert "FLOWHUB_NEXTCLOUD_URL" in EDITABLE_FIELDS
        assert "FLOWHUB_WOOCOMMERCE_URL" in EDITABLE_FIELDS
        assert "FLOWHUB_TIMEZONE" in EDITABLE_FIELDS
        assert "FLOWHUB_CURRENCY" in EDITABLE_FIELDS

    def test_known_installer_fields(self):
        assert "FLOWHUB_DOMAIN" in INSTALLER_ONLY_FIELDS
        assert "FLOWHUB_PORT" in INSTALLER_ONLY_FIELDS
        assert "FLOWHUB_DATABASE_URL" in INSTALLER_ONLY_FIELDS

    def test_known_secret_fields(self):
        assert "FLOWHUB_JWT_SECRET" in SECRET_RUNTIME_FIELDS
        assert "FLOWHUB_POSTGRES_PASSWORD" in SECRET_RUNTIME_FIELDS
        assert "FLOWHUB_NEXTCLOUD_PASSWORD" in SECRET_RUNTIME_FIELDS
        assert "FLOWHUB_WOOCOMMERCE_KEY" in SECRET_RUNTIME_FIELDS
        assert "FLOWHUB_WOOCOMMERCE_SECRET" in SECRET_RUNTIME_FIELDS


class TestConfigRecord:
    def test_to_dict_hides_secret_value(self):
        record = ConfigRecord(
            field_name="FLOWHUB_JWT_SECRET",
...
class TestConfigChangeEvent:
    ...
    def test_changed_by_default_is_cli(self):
        event = ConfigChangeEvent(field_name="FLOWHUB_LOG_LEVEL", old_value="INFO", new_value="DEBUG")
