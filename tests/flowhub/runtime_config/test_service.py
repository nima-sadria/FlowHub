...
from app.flowhub.runtime_config.service import RuntimeConfigService
from app.flowhub.runtime_config.record import EDITABLE_FIELDS, INSTALLER_ONLY_FIELDS, SECRET_RUNTIME_FIELDS


class TestGet:
    def test_get_editable_field(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        record = svc.get("FLOWHUB_LOG_LEVEL")
        assert record.field_name == "FLOWHUB_LOG_LEVEL"
    ...
    def test_get_secret_field_hides_value(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        record = svc.get("FLOWHUB_JWT_SECRET")
    ...
    def test_get_missing_field_returns_empty_value(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        record = svc.get("FLOWHUB_NONEXISTENT_FIELD")
        assert record.current_value == ""
...
class TestSet:
    def test_set_editable_field_success(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_LOG_LEVEL", "DEBUG")
    ...
    def test_set_persists_to_file(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        svc.set("FLOWHUB_LOG_LEVEL", "DEBUG")
        svc2 = RuntimeConfigService(env_file=env_file)
        record = svc2.get("FLOWHUB_LOG_LEVEL")
        assert record.current_value == "DEBUG"

    def test_set_updates_existing_line(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        svc.set("FLOWHUB_LOG_LEVEL", "WARNING")
        content = env_file.read_text(encoding="utf-8")
        lines_with_log = [l for l in content.splitlines() if l.startswith("FLOWHUB_LOG_LEVEL=")]
        assert len(lines_with_log) == 1
        assert lines_with_log[0] == "FLOWHUB_LOG_LEVEL=WARNING"

    def test_set_appends_if_field_missing(self, tmp_path: Path):
        env = tmp_path / ".env"
        env.write_text("FLOWHUB_ENV=production\n", encoding="utf-8")
        svc = RuntimeConfigService(env_file=env)
        result = svc.set("FLOWHUB_LOG_LEVEL", "DEBUG")
        assert result.success is True
        content = env.read_text(encoding="utf-8")
        assert "FLOWHUB_LOG_LEVEL=DEBUG" in content
    ...
    def test_set_returns_change_event(self, env_file: Path):
        ...
        assert result.change_event.field_name == "FLOWHUB_LOG_LEVEL"
    ...
    def test_set_normalizes_field_name(self, env_file: Path):
        ...
        result = svc.set("FLOWHUB_log_level", "DEBUG")
        assert result.success is True
        assert result.field_name == "FLOWHUB_LOG_LEVEL"

    def test_set_url_success(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_NEXTCLOUD_URL", "https://new.example.com")
        assert result.success is True

    def test_set_woocommerce_url_success(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_WOOCOMMERCE_URL", "https://newshop.example.com")
        assert result.success is True

    def test_set_timezone_success(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_TIMEZONE", "America/New_York")
        assert result.success is True

    def test_set_currency_success(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_CURRENCY", "EUR")
        assert result.success is True

    def test_set_scheduler_poll_seconds(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_SCHEDULER_POLL_SECONDS", "120")
        assert result.success is True

    def test_set_backup_retain_days(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_BACKUP_RETAIN_DAYS", "30")
        assert result.success is True
...
class TestSetRejections:
    ...
    def test_set_rejected_does_not_write_file(self, env_file: Path):
        ...
        svc.set("FLOWHUB_DOMAIN", "evil.example.com")
...
class TestValidation:
    ...
    def test_valid_3_letter_currency(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_CURRENCY", "IRR")
        assert result.success is True
    ...
    def test_4_letter_currency_rejected(self, env_file: Path):
        svc = RuntimeConfigService(env_file=env_file)
        result = svc.set("FLOWHUB_CURRENCY", "EURO")
        assert result.success is False
