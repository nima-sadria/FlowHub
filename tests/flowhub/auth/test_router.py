...
class TestLogin:
    ...
    def test_audit_event_on_success(self, client, admin_user, db):
        ...
        events = db.query(FlowHubLoginAudit).filter(FlowHubLoginAudit.event == "login_success").all()
        assert len(events) == 1
    ...
    def test_audit_event_on_failure(self, client, admin_user, db):
        client.post("/api/auth/login", json={"username": "testadmin", "password": "wrong"})
        from app.flowhub.auth.models import FlowHubLoginAudit
        events = db.query(FlowHubLoginAudit).filter(FlowHubLoginAudit.event == "login_failed").all()
        assert len(events) == 1
...
class TestRefresh:
    ...
    def test_audit_event_on_refresh(self, client, admin_user, db):
        ...
        client.post("/api/auth/refresh", json={"refresh_token": rt})
        from app.flowhub.auth.models import FlowHubLoginAudit
        events = db.query(FlowHubLoginAudit).filter(FlowHubLoginAudit.event == "token_refresh").all()
        assert len(events) == 1
...
class TestLogout:
    ...
    def test_audit_event_on_logout(self, client, admin_user, db):
        ...
        from app.flowhub.auth.models import FlowHubLoginAudit
        events = db.query(FlowHubLoginAudit).filter(FlowHubLoginAudit.event == "logout").all()
        assert len(events) == 1
