"""Tests for app.flowhub.config.schema (FlowHubConfig)."""

import pytest
from pydantic import ValidationError

from app.flowhub.config.profiles import ConfigProfile
from app.flowhub.config.schema import FlowHubConfig


class TestFlowHubConfigFromEnvValid:
    def test_builds_from_valid_env(self, valid_env):
        config = FlowHubConfig.from_env(valid_env)
        assert config.env == ConfigProfile.PRODUCTION
        assert config.domain == "test.example.com"
        assert config.port == 8080

    def test_env_field_maps_to_profile(self, valid_env):
        valid_env["FLOWHUB_ENV"] = "dev"
        config = FlowHubConfig.from_env(valid_env)
    ...
    def test_log_level_uppercased(self, valid_env):
        valid_env["FLOWHUB_LOG_LEVEL"] = "debug"
        config = FlowHubConfig.from_env(valid_env)
        assert config.log_level == "DEBUG"

    def test_banner_returns_string(self, valid_env):
        config = FlowHubConfig.from_env(valid_env)
        assert "PRODUCTION" in config.banner()
...
class TestFlowHubConfigFromEnvInvalid:
    def test_bad_ssl_mode_raises(self, valid_env):
        valid_env["FLOWHUB_SSL_MODE"] = "auto"
        with pytest.raises(ValidationError) as exc_info:
            FlowHubConfig.from_env(valid_env)
    ...
    def test_bad_database_url_raises(self, valid_env):
        valid_env["FLOWHUB_DATABASE_URL"] = "mysql://user:pass@host/db"
        with pytest.raises(ValidationError):
            FlowHubConfig.from_env(valid_env)
    ...
    def test_short_rest_api_secret_raises(self, valid_env):
        valid_env["FLOWHUB_REST_API_SECRET"] = "short"
        with pytest.raises(ValidationError):
            FlowHubConfig.from_env(valid_env)

    def test_empty_domain_raises(self, valid_env):
        valid_env["FLOWHUB_DOMAIN"] = ""
        with pytest.raises(ValidationError):
            FlowHubConfig.from_env(valid_env)
    ...
    def test_empty_postgres_password_raises(self, valid_env):
        valid_env["FLOWHUB_POSTGRES_PASSWORD"] = ""
        with pytest.raises(ValidationError):
            FlowHubConfig.from_env(valid_env)

    def test_bad_log_level_raises(self, valid_env):
        valid_env["FLOWHUB_LOG_LEVEL"] = "VERBOSE"
        with pytest.raises(ValidationError):
            FlowHubConfig.from_env(valid_env)
