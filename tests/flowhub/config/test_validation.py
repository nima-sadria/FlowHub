"""Tests for app.flowhub.config.validation."""

import pytest

from app.flowhub.config.validation import (
    ConfigValidator,
    FieldError,
    REQUIRED_FIELDS,
    ValidationResult,
...
class TestValidationResult:
    ...
    def test_add_error(self):
        r = ValidationResult()
        r.add_error("FLOWHUB_ENV", "bad", "not valid")
        assert len(r.errors) == 1
        assert r.errors[0].field == "FLOWHUB_ENV"
    ...
    def test_format_errors_with_error(self):
        r = ValidationResult()
        r.add_error("FLOWHUB_DOMAIN", "bad_val", "too short")
        formatted = r.format_errors()
        assert "FLOWHUB_DOMAIN" in formatted
        assert "too short" in formatted

    def test_format_errors_redacts_secrets(self):
        r = ValidationResult()
        r.add_error("FLOWHUB_JWT_SECRET", "my_secret", "too short")
        formatted = r.format_errors()
...
class TestFieldCheckers:
    ...
    def test_check_env_valid_values(self):
        for v in ("dev", "production", "production"):
...
class TestConfigValidatorFull:
    ...
    def test_path_check_nonexistent_path(self, valid_env, tmp_path):
        valid_env["FLOWHUB_STORAGE_PATH"] = str(tmp_path / "does_not_exist")
        validator = ConfigValidator(check_paths=True)
        result = validator.validate(valid_env)
        assert any(e.field == "FLOWHUB_STORAGE_PATH" for e in result.errors)

    def test_path_check_skipped_when_disabled(self, valid_env, tmp_path):
        valid_env["FLOWHUB_STORAGE_PATH"] = str(tmp_path / "does_not_exist")
        valid_env["FLOWHUB_BACKUP_PATH"] = str(tmp_path / "also_not_exist")
        validator = ConfigValidator(check_paths=False)
        result = validator.validate(valid_env)
        assert not any(e.field in ("FLOWHUB_STORAGE_PATH", "FLOWHUB_BACKUP_PATH") for e in result.errors)

    def test_multiple_errors_collected(self, valid_env):
        valid_env["FLOWHUB_PORT"] = "not_a_port"
        valid_env["FLOWHUB_CURRENCY"] = "xx"
        valid_env["FLOWHUB_TIMEZONE"] = "BadZone"
        validator = ConfigValidator(check_paths=False)
        result = validator.validate(valid_env)
