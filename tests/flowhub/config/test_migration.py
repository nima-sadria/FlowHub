"""Tests for app.flowhub.config.migration."""

import pytest

from app.flowhub.config.migration import CURRENT_CONFIG_VERSION, ConfigMigration


class TestConfigMigration:
    def test_detect_version_present(self):
        m = ConfigMigration()
        config = {"meta": {"version": "FLOWHUB-1.0.0"}}
        assert m.detect_version(config) == "FLOWHUB-1.0.0"
    ...
    def test_no_migration_for_current_version(self):
        m = ConfigMigration()
        config = {"meta": {"version": CURRENT_CONFIG_VERSION}, "app": {"env": "production"}}
    ...
    def test_current_version_constant(self):
        assert CURRENT_CONFIG_VERSION == "FLOWHUB-1.0.0"
