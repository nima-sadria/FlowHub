"""Tests for DiagnosticRunner - all using TestDoubleNetworkAdapter, no real network."""
...
from app.flowhub.diagnostics.report import DiagnosticReport
from app.flowhub.health.models import HealthStatus


@pytest.fixture
def test_double() -> TestDoubleNetworkAdapter:
    return TestDoubleNetworkAdapter()


@pytest.fixture
def runner(test_double: TestDoubleNetworkAdapter) -> DiagnosticRunner:
    return DiagnosticRunner(adapter=test_double, config={})
...
class TestRunIntegration:
    def test_all_checks_pass_gives_pass_report(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
    ...
    def test_dns_failure_gives_fail_report(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
    ...
    def test_dns_fail_causes_downstream_skips(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
        test_double.dns_responses["nextcloud.example.com"] = DNSResolutionError("NXDOMAIN")
        report = runner.run_integration("nextcloud", url="https://nextcloud.example.com")
    ...
    def test_tls_failure_gives_fail_report(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
        test_double.tls_responses[("nextcloud.example.com", 443)] = TLSHandshakeError("cert error")
        report = runner.run_integration("nextcloud", url="https://nextcloud.example.com")
        assert report.overall_status == HealthStatus.FAIL
        assert report.overall_failure_class == FailureClass.TLS_FAILURE

    def test_timeout_gives_fail_report(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
        test_double.dns_responses["nextcloud.example.com"] = ConnectionTimeoutError("timeout")
        report = runner.run_integration("nextcloud", url="https://nextcloud.example.com")
        assert report.overall_status == FailureClass.TIMEOUT or report.overall_status == HealthStatus.FAIL

    def test_auth_failure_gives_fail_report(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
        test_double.auth_responses["https://nextcloud.example.com"] = AuthenticationError("401")
    ...
    def test_secrets_not_in_report(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
        report = runner.run_integration(
            "nextcloud",
            url="https://nextcloud.example.com",
            auth_username="admin",
    ...
    def test_fail_report_has_repair_steps(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
        test_double.dns_responses["nextcloud.example.com"] = DNSResolutionError("NXDOMAIN")
        report = runner.run_integration("nextcloud", url="https://nextcloud.example.com")
    ...
    def test_unexpected_exception_caught_as_unknown_error(self, test_double: TestDoubleNetworkAdapter):
        class BoomAdapter(TestDoubleNetworkAdapter):
    ...
    def test_credentials_from_config_dict(self, test_double: TestDoubleNetworkAdapter):
        config = {
            "FLOWHUB_NEXTCLOUD_URL": "https://nextcloud.example.com",
            "FLOWHUB_NEXTCLOUD_USERNAME": "user",
            "FLOWHUB_NEXTCLOUD_PASSWORD": "pass",
        }
        runner = DiagnosticRunner(adapter=test_double, config=config)
        report = runner.run_integration("nextcloud")
        assert report.target == "nextcloud"

    def test_url_fallback_to_config(self, test_double: TestDoubleNetworkAdapter):
        config = {"FLOWHUB_NEXTCLOUD_URL": "https://nextcloud.example.com"}
        runner = DiagnosticRunner(adapter=test_double, config=config)
        report = runner.run_integration("nextcloud")
        assert report.overall_status == HealthStatus.PASS
...
class TestRunAll:
    def test_run_all_covers_all_known_services(self, runner: DiagnosticRunner, test_double: TestDoubleNetworkAdapter):
    ...
    def test_run_all_unexpected_exception_caught(self, test_double: TestDoubleNetworkAdapter):
        class PartialBoomAdapter(TestDoubleNetworkAdapter):
            _call_count = 0
...
class TestReportSeverityOrdering:
    def test_worst_failure_class_selected(self, test_double: TestDoubleNetworkAdapter):
        test_double.dns_responses["nextcloud.example.com"] = DNSResolutionError("NXDOMAIN")
        runner = DiagnosticRunner(adapter=test_double, config={})
        report = runner.run_integration("nextcloud", url="https://nextcloud.example.com")
        assert report.overall_failure_class == FailureClass.DNS_FAILURE
        assert report.overall_severity.value in ("error", "critical", "warning", "degraded")

    def test_warn_does_not_override_fail(self, test_double: TestDoubleNetworkAdapter):
        test_double.dns_responses["woocommerce.example.com"] = DNSResolutionError("NXDOMAIN")
        runner = DiagnosticRunner(adapter=test_double, config={})
