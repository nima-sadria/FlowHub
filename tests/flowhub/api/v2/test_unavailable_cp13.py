"""Tests for CP1.3 REST API contract unavailables - shape and importability."""
...
class TestDiagnosticsUnavailable:
    def test_router_importable(self):
        from app.flowhub.api.v2.diagnostics import router
    ...
    def test_run_request_model_importable(self):
        from app.flowhub.api.v2.diagnostics import DiagnosticRunRequest
    ...
    def test_repair_step_shape_importable(self):
        from app.flowhub.api.v2.diagnostics import RepairStepShape
    ...
    def test_response_model_importable(self):
        from app.flowhub.api.v2.diagnostics import DiagnosticRunResponse
        assert DiagnosticRunResponse is not None


class TestHealthV2Unavailable:
    def test_router_importable(self):
        from app.flowhub.api.v2.health import router
        assert isinstance(router, APIRouter)
    ...
    def test_on_demand_request_model(self):
        from app.flowhub.api.v2.health import OnDemandCheckRequest
        req = OnDemandCheckRequest(target="nextcloud")
        assert req.target == "nextcloud"

    def test_control_plane_status_shape(self):
        from app.flowhub.api.v2.health import ControlPlaneStatusShape
        assert ControlPlaneStatusShape is not None


class TestConfigV2Unavailable:
    def test_router_importable(self):
        from app.flowhub.api.v2.config import router
        assert isinstance(router, APIRouter)
    ...
    def test_set_request_model_importable(self):
        from app.flowhub.api.v2.config import ConfigSetRequest
        req = ConfigSetRequest(value="DEBUG")
        assert req.value == "DEBUG"

    def test_record_shape_importable(self):
        from app.flowhub.api.v2.config import ConfigRecordShape
