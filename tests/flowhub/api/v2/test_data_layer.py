...
os.environ.setdefault("FLOWHUB_JWT_SECRET", "test-dl-jwt-secret-exactly-32-bytes!")

# Register all ORM models so FlowHubBase.metadata.create_all covers them.
...
def auth_headers(client, db):
    ...
    user = FlowHubUser(username="dladmin", hashed_password=hash_password("dlpass1234"), role="admin")
    db.add(user)
    db.commit()
...
class TestRefreshJobService:
    def test_empty_summary(self, db):
        from app.flowhub.data_layer.refresh_service import RefreshJobService
...
class TestInvalidationService:
    def test_empty_summary(self, db):
        from app.flowhub.data_layer.invalidation_service import InvalidationService
...
class TestNoWritePaths:
    ...
    def test_router_does_not_import_httpx_directly(self):
        import ast
        import pathlib
        src = pathlib.Path(__file__).parents[4] / "app" / "flowhub" / "api" / "v2" / "data_layer_routes.py"
    ...
    def test_service_modules_do_not_import_httpx(self):
        import ast
        import pathlib
        dl_dir = pathlib.Path(__file__).parents[4] / "app" / "flowhub" / "data_layer"
